...
class Master(object):
    def __init__(self, customer_listen_addr, communicate_addr, port_files,
    ...
    def _listen_slaver(self):
        ...
        with open(self.vm_port_file, "w") as vm_file:
            vm_file.write(str(sock.getsockname()[1]))
        sock.listen(10)
    ...
    def _listen_customer(self):
        ...
        with open(self.xpra_port_file, "w") as xpra_port_file:
            xpra_port_file.write(str(sock.getsockname()[1]))
        print('xpra port', sock.getsockname()[1])
        sock.listen(20)
        _listening_sockets.append(sock)
...
def run_master(communicate_addr, customer_listen_addr, port_files, ssl=False):
    ...
    Master(customer_listen_addr, communicate_addr, port_files, ssl=ssl).serve_forever()
...
""",
    ...
    parser.add_argument("-x", "--xpraportfile", type=str, dest="XPRA_PORT_FILE", required=True)
    parser.add_argument("-z", "--vmportfile", type=str, dest="VM_PORT_FILE", required=True)
...
def main_master():
    ...
    communicate_addr = ("0.0.0.0", 0) # port client shootback connects to
    customer_listen_addr = ("127.0.0.1", 0) # port xpra connects to

    port_files = [args.XPRA_PORT_FILE, args.VM_PORT_FILE]

    print(communicate_addr, customer_listen_addr, port_files)
    ...
    try:
        run_master(communicate_addr, customer_listen_addr, port_files, ssl=args.ssl)
    finally:
