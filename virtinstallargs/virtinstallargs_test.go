...
import (
	...
	"git.voidnet.tech/kev/easysandbox/libvirtsb"
)
...
func TestGetVirtInstallArgs(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: []string{
				"--name", "testdomain",
				"--disk", libvirtsb.DomainsDir + "testdomain" + "/" + "root.qcow2" + ",target.bus=sata",
				"--disk", libvirtsb.DomainsDir + "testdomain" + "/" + "home.qcow2" + ",target.bus=sata",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := libvirtsb.GetVirtInstallArgsString(tt.args.domainName, tt.args.args...); !reflect.DeepEqual(got, tt.want) {
